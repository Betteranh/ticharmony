...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    // ─── Requêtes par profil (CLIENT) ─────────────────────────────────────────

    List<Notification> findByUserProfileAndViewedFalseOrderByCreatedAtDesc(UserProfile profile);

    List<Notification> findByUserProfileAndProblemAndTypeAndViewedFalse(
            UserProfile profile, Problem problem, NotificationType type);

    @Query("SELECT COUNT(n) FROM Notification n WHERE n.userProfile = ?1 AND n.viewed = false")
    long countUnreadByUserProfile(UserProfile profile);

    @Modifying
    @Query("UPDATE Notification n SET n.viewed = true WHERE n.userProfile = ?1 AND n.viewed = false")
    int markAllReadByUserProfile(UserProfile profile);

    @Modifying
    @Query("UPDATE Notification n SET n.viewed = true WHERE n.userProfile = ?1 AND n.problem = ?2 AND n.viewed = false")
    int markAllReadByUserProfileAndProblem(UserProfile profile, Problem problem);
}